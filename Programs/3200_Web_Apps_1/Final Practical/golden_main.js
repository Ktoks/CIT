...
getButton.onclick = function() {
    ...
    if (name.value == '') {
        ...
        alert("Please input your guest's name!")
    }
    else {
...
var postTicket = function (name, age, guestName) {
    ...
    fetch("http://localhost:8080/tickets", {
        ...
        if (response.status == 403) {
            alert('The Oompa Loompas have already recieved your ticket. Please try again tomorrow.');
        }
    });
}
...
var loadTickets = function() {
    fetch("http://localhost:8080/tickets",{
        ...
        if (response.status == 200) {
            ...
            console.log("You have already made a ticket.");
        } else {
